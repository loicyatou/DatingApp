...
export class MembersService {
  ...
  private getPaginatedResults<T>(url: string, params: HttpParams) {
    ...
    return this.http.get<T>(url, { observe: 'response', params }).pipe(
      map(response => {
        if (response.body) {
          paginatedResult.result = response.body; //passes the results of the query to the results in pagination angular class
        }
        const pagination = response.headers.get('Pagination');
        if (pagination) {
          paginatedResult.pagination = JSON.parse(pagination); //passes the pagination details to the pagination angular class. this will be usedto split the pages up
        }
